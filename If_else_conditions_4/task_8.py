...
def next_date(day: int, month: int) -> str:
    """Возвращает строку с датой следующего дня после заданной даты (невисокосный год)."""

    try:
        d = int(day)
        m = int(month)
    except (TypeError, ValueError):
        return "Ошибка-ввода: значения для дней и месяца должны быть целыми числами"

    match d, m:
        case (d, m) if 1 <= d < 30 and m in [4, 6, 9, 11]:  # Месяцы с 30 днями
            return f"Следующая дата: {d + 1}.{m}"
        case (30, m) if m in [4, 6, 9, 11]:  # Переход на следующий месяц в месяце с 30 днями
            return f"Следующая дата: 1.{m + 1}"
        case (31, 12):  # Переход на следующий год
            return "Следующая дата: 1.1\nC новым годом!"
        case (d, m) if 1 <= d < 31 and m in [1, 3, 5, 7, 8, 10, 12]:  # Месяцы с 31 днем
            return f"Следующая дата: {d + 1}.{m}"
        ...
        case (28, 2):  # Переход на следующий месяц в феврале
            return f"Следующая дата: 1.{m + 1}"
        case _:
            return "Ошибка-ввода: значение даты некорректно"
